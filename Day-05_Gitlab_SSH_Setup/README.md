...
### **SSH (e.g. `git@gitlab.devopsinaction.lab:root/runner-test.git`)**

* Uses **SSH keys** for authentication.
* You generate a key pair (`id_rsa` + `id_rsa.pub`) and add the public key to your GitLab account.
* Once that’s done, Git operations (clone, push, pull, etc.) **don’t ask for username/password** every time.
* Authentication happens automatically and securely via SSH.

### **HTTPS (e.g. `https://gitlab.devopsinaction.lab/root/runner-test.git`)**

* Uses **username/password** or **personal access token (PAT)** for authentication.
* You may have to enter credentials each time unless you cache them or use a credential helper.
* More convenient for initial use (no SSH setup) but less smooth for long-term frequent use.
...
| Feature                         | SSH                                                   | HTTPS                                                   |
| ------------------------------- | ----------------------------------------------------- | ------------------------------------------------------- |
| **Authentication**              | SSH keys (public/private pair)                        | Username + password or token                            |
| **Convenience**                 | No need to re-enter credentials                       | Might prompt often (unless cached)                      |
| **Security**                    | Strong, key-based auth (no password over the network) | Also secure (TLS), but depends on credential management |
| **Automation (CI/CD, scripts)** | Excellent — can use deploy keys                       | Requires token management                               |
| **Firewall issues**             | May need port 22 open                                 | Works anywhere HTTPS is allowed                         |



## ⚙️ Typical Usage Patterns

* **Developers working regularly with GitLab:** prefer SSH (once set up, no extra login hassle).
* **Temporary access or first-time use:** HTTPS might be simpler (no key setup needed).
* **Automated environments (CI/CD, servers):** SSH with deploy keys is more secure and script-friendly.
...
| ----------------------------------------- | ----------- |
| You’re setting up a CI runner or server   | **SSH**     |
| You frequently push/pull code             | **SSH**     |
| You just want to clone once to read code  | **HTTPS**   |
| You’re behind a firewall blocking port 22 | **HTTPS**   |


✅ **Benefit:**

* No password prompts after initial SSH key setup.
* Works well for automation and regular use.
